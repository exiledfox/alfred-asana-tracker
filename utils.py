...
from asana.api import set_field_value, get_field_value, get_task
...
def stop_task(wf, task_gid):
    ...
    task_history.append({
        "task": {
            ...
            if k in ["name", "gid", "projects", "permalink_url"]
        },
...
def generate_report(wf, query):
    ...
    for i in range(len(task_history)):
        ...
        rows[task["gid"]]["Hours"] += (
            item["end_timestamp"]
            - max(item["start_timestamp"], min_timestamp)
        )/3600
    ...
    command = (
        ...
        + path.replace(" ", "\ ")
        + " -c .md -g "
        + "/Users/" + user.replace(" ", "\ ") + "/Library/QuickLook/QLMarkdown.qlgenerator"
        + " >/dev/null 2>&1 &"
    )
    os.system(command)
